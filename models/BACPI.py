...
class BACPI(nn.Module):
    def __init__(self, n_output, comp_dim, prot_dim, gat_dim, num_head, dropout, alpha, window, layer_cnn, latent_dim, layer_out, **config):
        ...
        self.embedding_layer_atom = nn.Embedding(len(atom_dict), comp_dim)
        self.embedding_layer_amino = nn.Embedding(len(amino_dict), prot_dim)
        ...
        self.output = nn.Linear(latent_dim * latent_dim * 2, n_output)
    ...
    def forward(self, data):
        data = data[0]
        atoms, atoms_mask, adjacency, amino, amino_mask, fps = data.x, data.x_mask, data.adj, data.prot, data.prot_mask, data.fps
