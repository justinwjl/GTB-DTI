...
class GCNNet_pretrain(GraphDTA):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128,num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.2, **config):

        super(GCNNet_pretrain, self).__init__()
        # 78 91 102 106 109 110
        # num_features_xd = 78
        # SMILES graph branch
        self.n_output = n_output
        ...
        self.embedding_xt = nn.Embedding(num_features_xt + 1, embed_dim)
        self.conv_xt_1 = nn.Conv1d(in_channels=1000, out_channels=n_filters, kernel_size=8)
        self.conv_xt_2 = nn.Conv1d(in_channels=320, out_channels=n_filters, kernel_size=8)
        self.fc1_xt = nn.Linear(32*121, output_dim)
        self.fc2_xt = nn.Linear(320, output_dim)
    ...
    def _forward_protein(self, molecules):
        # embedded_mol = self.embedding_xt(molecules)
        # results = self.esm_model(molecules, repr_layers=[33], return_contacts=True)
        # token_representations = results["representations"][33]
        # conv_xt = self.conv_xt_1(embedded_mol)
        # conv_xt = self.conv_xt_1(token_representations)
        # flatten
        # xt = conv_xt.view(-1, 32 * 121)
        # conv_xt = self.conv_xt_2(molecules)
        # xt = conv_xt.view(-1, 32 * 121)
        xt = molecules
        feature = self.fc2_xt(xt)
        return feature
